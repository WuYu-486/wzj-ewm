...
class TeacherMateWebSocketClient:
    ...
    def __init__(self, sign_id: int, qr_callback: Optional[Callable] = None,
        ...
        # 事件回调：把「会话结束 / 二维码暂不可用」这类协议事件上报给调用方。
        # 默认 None，桌面版（run.py）不传，行为与改动前完全一致。
    ...
    def _report_event(self, event_type: int, **extra: Any) -> None:
        """上报协议事件。没有传 event_callback 时什么都不做"""
        if not self.event_callback:
            return
        try:
            payload: Dict[str, Any] = {"type": event_type, "sign_id": self.sign_id}
    ...
    async def _handle_qr_message(self, message: str) -> None:
        ...
        try:
            ...
            if isinstance(qr_data, list) and len(qr_data) > 0:
                if not isinstance(qr_data[0], dict) or "data" not in qr_data[0]:
                    return
                ...
                if qr_code_data["type"]==1:
                    ...
                    self._report_event(1, qr_url=qr_code_url)
                elif qr_code_data["type"]==3:
                    logger.info("qr_url为空，前方拥挤")
                    # 瞬态拥挤，不结束任何东西，只上报
                    self._report_event(3)
                elif qr_code_data["type"]==2:
                    logger.info(f"检测到关闭信息")
                    # 先上报，再保持原有的自毁语义（桌面版行为不变）
                    self._report_event(2)
